#!/usr/bin/env bash
# Enforces the memory vault's write zones, so the convention in Home.md is a gate
...
jq -nc --arg reason "This edits a curated memory note. Home.md routes these through the memory-groom skill or an explicit request; confirm it was wanted." \
